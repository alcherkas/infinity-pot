...
verdict on trello-clone turn 13 task-017 edit: KEPT — following the added instruction, this run executed `npx playwright test` after implementing task-017 (NFR7 save-warning banner), confirming all 10/10 existing specs still passed before finishing; the check would have caught any DnD/locator regression from the new banner markup the same way it caught the task-010 regression.

### trello-clone turn 13 — release-manager [PROBATION]
before: "Declare GO only if all gates are APPROVED, every backlog task is DONE, and no OPEN bugs remain — otherwise NO-GO."
after: "Declare GO only if all gates are APPROVED, every Must/Should-scope backlog task is DONE (Could-have rows explicitly marked TODO by deliberate MoSCoW deferral in backlog.md's own notes don't block), and no OPEN bugs remain — otherwise NO-GO."
evidence: trello-clone's own backlog.md rows 14-16 (task-012/013/014, Could-have) are TODO by design per requirements.md's MoSCoW scoping and the backlog's own "Reordering notes" explicitly deferring them behind ship-readiness — a literal reading of "every backlog task is DONE" would have forced a spurious NO-GO on a project where every Must/Should FR is built, reviewed, and verified, and the deferral itself is documented, not an oversight.
