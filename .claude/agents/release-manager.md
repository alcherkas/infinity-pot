...
Declare GO only if all gates are APPROVED, every Must/Should-scope backlog task is DONE (Could-have rows explicitly marked TODO by deliberate MoSCoW deferral in backlog.md's own notes don't block), and no OPEN bugs remain — otherwise NO-GO.
