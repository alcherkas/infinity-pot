...
This repository is an orchestrator/worker multi-agent example. In this repo the **main session is the ORCHESTRATOR** — a pure control plane. Products are built in `workspace/<slug>/` by the 20 subagents defined in `.claude/agents/`, launched with the Agent tool. The single entry point is the `/pot` command.
...
5. **Reflection is decentralized.** The orchestrator never edits `.claude/agents/` — agent definitions improve only through each worker's own self-reflection at the end of its run (see Worker protocol). The orchestrator's only duty is to make sure `scribe` carries this turn's `reflections.md` entries into the log.
...
- **Self-reflection (end of every run):** before your final line, judge whether your own definition (`.claude/agents/<your-name>.md`) misled or under-specified you during this run. If it did, rewrite the offending sentence(s) in your own file — the whole definition stays ≤10 sentences, frontmatter (`name`, `description`, `tools`) and your core role are immutable, and you never edit another agent's file. Then append one entry to the project's `reflections.md` (append-only): `Turn N — <agent>: "<before>" → "<after>" — <why>`. If nothing needs improving, change nothing — most runs should be no-ops.
...
4. **Record** — orchestrator updates `state.json` (turn+1, last_action, next), then launches `scribe`, which appends the turn entry to `log.md` (sections: Done / Walkthrough / Agent improvements — the latter from this turn's `reflections.md` entries) and makes the turn's single commit.
...
├── reflections.md    # append-only: workers' self-reflection notes (before→after + why)
